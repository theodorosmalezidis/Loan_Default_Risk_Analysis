...
|------------------|---------------|-----------------------------------------------------------------------------------------------|
| borrower_id       | NVARCHAR(50) primary key           | A unique identifier for each borrower in the dimension table.               |
| age        | INT           | The age of the borrower.                                        |
| state | NVARCHAR(50)  | The borrower's state of residence.         |
| education_level          | NVARCHAR(50)  | The borrower's education level (5 distinct values)      .                                  |
| employment_status    | NVARCHAR(50)  | Current employment status of the borrower (5 distinct values)     .                                                |
...
| home_ownership      | NVARCHAR(50)  |The borrower's current home ownership status (3 distinct values).               |
...
| Column Name      | Data Type     | Description                                                                                   |
|------------------|---------------|-----------------------------------------------------------------------------------------------|
| loan_id       | NVARCHAR(50) primary key           | A unique id for each loan record in the fact table.               |
|  borrower_id    | NVARCHAR(50) foreign key           |A unique identifier that connects the borrowers_dim table to the loans_fact table.                                        |
| application_date | DATE  | The date of the loan's application.         |
| loan_purpose          | NVARCHAR(50)  | The official purpose stated for the loan application .                                  |
...
| interest_rate      | DECIMAL (3, 2)        | The annual percentage charged  on the principal loan amount.
...
| dti_ratio      | DECIMAL (3, 2)  |The percentage of the borrower's gross monthly income that goes toward paying monthly debt obligations.               |
| loan_status           | NVARCHAR(50)  | The current repayment status of the loan, classified based on the number of days elapsed since the last missed or overdue payment (4 distinct values).|
